...
router.get('/accounts', (request, response, next) => {
    ...
    allAccounts({ adminID })
        .then((items = []) => response.json({
            status: 200,
            items: items.map(({ _id, name, author, }) => ({ id: _id, name }))
        }))
        .catch(next)
})
...
router.put('/accounts/:accountID/users/:userID/add.number', (request, response, next) => {
    const { adminID, params: { accountID, userID }, body: { number } } = request

    addPhoneNumber({ adminID, userID, accountID, number })
        .then(number => response.json({ status: 200, number }))
        .catch(next)
})

router.put('/accounts/:accountID/users/:userID/remove.number', (request, response, next) => {
    const { adminID, params: { accountID, userID }, body: { number } } = request

    removePhoneNumber({ adminID, userID, accountID, number })
        .then(() => response.json({ status: 200 }))
        .catch(next)
})

router.put('/accounts/:accountID/users/:userID/edit.number', (request, response, next) => {
    const { adminID, params: { accountID, userID }, body: { number, newNumber } } = request
